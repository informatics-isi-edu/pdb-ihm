...
if [ -s ${ERROR_LOG_FILE} ] ;
then
	sudo -u pdbihm python3 -m pdb_dev.tools.send_email_notification --config /home/pdbihm/.secrets/mail.json -s "Error rsync archive" -b ${ERROR_LOG_FILE}
else
	sudo -u pdbihm python3 -m pdb_dev.tools.send_email_notification --config /home/pdbihm/.secrets/mail.json -s "Successfully rsync archive" -b ""
fi

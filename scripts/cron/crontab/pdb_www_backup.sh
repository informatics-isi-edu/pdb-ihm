...
FROM_CONFIG_WWW=${FROM_CONFIG}/www/.
...
echo "rsync -arv -e \"ssh -i ${IDENTITY_FILE}\" ${FROM_CONFIG_WWW} ${REMOTE_USER}@${REMOTE_HOST}:${TO_CONFIG_WWW}" >> ${BACKUP_LOG_FILE}
rsync -arv -e "ssh -i ${IDENTITY_FILE}" ${FROM_CONFIG_WWW} ${REMOTE_USER}@${REMOTE_HOST}:${TO_CONFIG_WWW} >>${OUTPUT_LOG_FILE} 2>>${ERROR_LOG_FILE}

...
def main():
    ...
    job_streams = [
        EntryJobStream(
            '/entity/M:=PDB:entry/Manual_Processing=False/Process_Status=any(%s)/Workflow_Status=any(%s)/$M?limit=1' % (process_status_string, workflow_status_string),
        ...
        RestraintJobStream(
            '/entity/M:=PDB:Entry_Related_File/Restraint_Process_Status=any(%s)/Restraint_Workflow_Status=DEPO/PDB:entry/Manual_Processing=False/Process_Status=%s/$M?limit=1' % (process_status_string, urlquote(process_status_terms['SUCCESS'])),

...
def main():
    ...
    job_streams = [
        EntryJobStream(
            '/attribute/M:=PDB:entry/Manual_Processing=False/Process_Status=any(%s)/Workflow_Status=any(%s)/$M/RID,id,Workflow_Status,Process_Status,mmCIF_File_Name?limit=1' % (process_status_string, workflow_status_string),
        ...
        RestraintJobStream(
            ...
            # -- sort by Rank
            '/attribute/M:=PDB:Entry_Related_File/Restraint_Process_Status=any(%s)/Restraint_Workflow_Status=DEPO/PDB:entry/Manual_Processing=False/Process_Status=%s/$M/F:=Vocab:File_Type/$M/RID,structure_id,File_Type,File_Name,F:Rank@sort(Rank,RID)?limit=1' % (process_status_string, urlquote(process_status_terms['SUCCESS'])),            

...
from .replace_pdb_ids_cif import process_cif_file, convert_pdb_id
from .replace_pdb_ids_json import process_json_file 
...
class PDBBetaArchiveError(ProcessingError):
    """ Error while generating PDB Beta Archive
    """
    pass
...
class ArchiveClient (PipelineProcessor):
    ...
    def rollbackArchive(self):
        ...
        try:
            ...
            subject = 'Error archiving files. Got exception during rollback.'            
            self.log_exception(e, notify=self.notify, subject=subject)
    ...
    def processArchive(self):
        ...
        try:
            ...
            initilize
            """
            #raise PDBBetaArchiveError("Die here: Test exception")
            #raise Exception("Die here: Test exception")            
            self.set_system_generated_file_types()
            self.set_pdb_archive()        
            self.set_entry_latest_archive()
    ...
    def archiveFiles(self):
        ...
        for rid, row in self.archive_entries.items():
            ...
            for file_type,file_generated in self.entry_generated_files[rid].items():
                ...
                file_path = self.getHatracFile(file_url, self.data_scratch, renamed_file)
                # Zip the file
        ...
        if self.processing_issues["incorrect filenames"] or self.processing_issues["missing files"]:
            subject = f'weekly archive errors/warnings: {", ".join(subject_rids)}'
    ...
    def getHatracFile(self, file_url, input_dir, filename):
        ...
        try:
            ...
            raise FileError("Error retrieving Hatrac file", details="Unable to retrieve hatrac file from url %s to file %s/%s." % (file_url, input_dir, filename))
    ...
    def generate_pdb_beta_archive(self):
        ...
        try:
            ...
            for root, dirs, files in os.walk(src_dir):
                # 'root' is the current directory path being visited.
                ...
                if m:   # rename entry folder
                    id = m[1]
                    converted_id = convert_pdb_id(id)
                    new_root = new_root.replace(id, converted_id)
                    
                rename_dirs.append( (root, new_root) )

                # Print files
                for f in files:
                    fp = os.path.join(root, f)
                    new_fp = os.path.join(new_root, f)
                    if f in ["current_file_holdings.json.gz", "released_structures_last_modified_dates.json.gz", "unreleased_entries.json.gz"]:
                        holding_files.append( (fp, new_fp) )
                    else:
                        m = re.search(r"^([0-9][0-9a-z]{3})[_\.](.*)", f)
                        if not m: raise Exception(f"ERROR: !!!!!! unexpected file found: {fp}")
                        id = m[1]
                        converted_id = convert_pdb_id(id)                            
                        new_f = f.replace(id, converted_id)
                        new_fp = os.path.join(new_root, new_f)
                        #print(f"  File: {fp}  --> {new_fp} ")                            
                        if f.endswith(".cif.gz"):
                            cif_files.append( (fp, new_fp) )
                        else:
                            rename_files.append( (fp, new_fp) )

            if self.verbose: print("-------------- beta archive summary -------------")
            # create-folders
            #print("rename_dirs: %s" % (json.dumps(rename_dirs, indent=4)))            
            for (src_dir, dest_dir) in rename_dirs:
                if self.verbose: print(f"--d create dir: \n {src_dir} --> \n {dest_dir}")
                os.makedirs(dest_dir, exist_ok=True)
            
            # copy validation reports
            #print("copy_files: %s" % (json.dumps(rename_files, indent=4)))            
            for (src_fp, dest_fp) in rename_files:
                if self.verbose: print(f"--cp cp files:\n {src_fp} --> \n {dest_fp}")
                shutil.copy(src_fp, dest_fp)
        
            # process holding files
            for (src_fp, dest_fp) in holding_files:
                if self.verbose: print(f"--h holding file:\n {src_fp} --> \n {dest_fp}")
                process_json_file(src_fp, dest_fp)
            
            # process cif files
            for (src_fp, dest_fp) in cif_files:
                if self.verbose: print(f"--c cif file\n {src_fp} --> \n {dest_fp}")                
                process_cif_file(src_fp, dest_fp)
            
        except Exception as e:
            print(f"An error occurred: {e}")            
            raise PDBBetaArchiveError(e)

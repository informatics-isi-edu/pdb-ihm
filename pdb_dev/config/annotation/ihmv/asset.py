...
def update_IHMV(model):
    ...
    schema.tables["Generated_File"].columns["File_URL"].asset.update({
        "md5": "File_MD5",
        "url_pattern": "%s/ihmv/generated/uid/{{#if _Entry_RCB}}{{#regexFindFirst _Entry_RCB \"[^/]+$\"}}{{this}}{{/regexFindFirst}}{{else}}{{#regexFindFirst $session.client.id \"[^/]+$\"}}{{this}}{{/regexFindFirst}}{{/if}}/structure/rid/{{{Structure_mmCIF}}}/validation_report/{{{File_Name}}}" % (cfg.hatrac_root),
        "filename_column": "File_Name",
        "byte_count_column": "File_Bytes"
    })
